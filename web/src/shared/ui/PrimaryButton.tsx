...
const OutlineLink = ({ children, href, className }: PrimaryLinkProps) => (
  <Link
    href={href}
    className={cn(
      "btn-border-slide inline-flex items-center justify-center gap-2 border border-foreground text-foreground hover:text-white [--btn-border-slide-fill:hsl(var(--foreground))] px-8 py-3 text-sm tracking-widest uppercase font-body",
      className
    )}
  >
    {children}
