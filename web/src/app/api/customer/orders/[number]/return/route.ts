...
import { requestCustomerOrderReturn } from "@/services/customer/order-actions.service";
import {
  OrderActionError,
...
/** Return request already settled server-side — the client refreshes and moves on. */
const CONFLICT_CODES: OrderActionErrorCode[] = ["ALREADY_REQUESTED", "NOT_RETURNABLE"];
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!orderId || !reason) {
    return NextResponse.json({ error: "Order and return reason are required." }, { status: 400 });
  }

  try {
    const result = await requestCustomerOrderReturn(token, { orderId, reason, comment });
